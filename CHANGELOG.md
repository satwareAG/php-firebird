...
- **debug/ASAN builds: runtime violated its own handwritten arginfo (every test BORKed at SKIPIF)**: debug PHP builds enforce internal arginfo/zpp consistency that release builds tolerate. Two latent violations since #306 (2026-07-05) made the whole suite unrunnable under `php83-asan`: (1) `fbird_query(FBIRD_CREATE, ...)` returned a raw `resource` while the arginfo mask declares `Firebird\ResultSet|int|bool` - it now returns a `Firebird\Connection` object, Phase-C parity with `fbird_connect()` (refcounting: the regular_list entry owns the register ref, the object owns setup's strong ref, no caller ref to drop); (2) `fbird_drop_db()` arginfo declared 1 param while the implementation accepts the `(dsn, user, pass)` string overload - "Arginfo / zpp mismatch" fatal on 2-3 arg calls. Stale `--EXPECTF--` of `fbird_drop_db_001/003/004` updated. `scripts/test.sh` no longer force-adds `-d extension=pcntl` when the container already provides it (AGENTS.md "pcntl double-loading"). Remaining debug-only latents filed: #594 (query-resource dtor reads freed transaction at shutdown), #595 (`fbird_connect()` arginfo missing the 9th `sync` param - in release builds `FBIRD_CONNECT_FORCE_NEW` passed as 8th arg silently sets the lock timeout instead), #596 (`fbird_drop_db_003` skipif SIGSEGV under ASAN), #593 (ZTS: statement sweep registry is thread_local).


- **#591: query result resources SIGSEGV at destruction when their connection died first**: `fbird_drop_db()` and `fbird_close()`/`fbc_disconnect()` delete the `fb::Connection` (RAII releases the `IAttachment` and every dependent interface), but query result resources still referenced by userland kept `StatementWrapper`s with dead `statement_`/`result_set_` pointers. At request shutdown the result dtor called `fbs_close_cursor`/`fbs_free` on those interfaces - heap-use-after-free (SIGSEGV in `StatementWrapper::closeCursor`, Termsig=11; deterministic with server-writable `FIREBIRD_DB_DIR`, e.g. CI `/tmp`; blocked PR #590). Fix: a per-thread sweep registry keyed by attachment identity (`fbs_prepare` registers, `fbs_free` unregisters); `fbc_disconnect()`/`fbc_drop_database()` neutralize dependent wrappers on every exit path (null handles without `release()` - the interfaces died with the attachment), so the existing null-guards in `closeCursor()`/`free()` turn later dtor calls into no-ops. Mirrors the transaction-handle nulling `fbird_drop_db()` already performed at the C layer. `BlobWrapper`/batch wrappers share the lifetime coupling but show no crash; extend the same pattern if one appears.

- **#582: stale persistent-link entry freed `fb_link` under live wrappers (heap-use-after-free)**: when a same-DSN `fbird_pconnect()` found the cached `persistent_list` entry with a dead server attachment (e.g. after `fbird_drop_db()`, or a lost connection), it deleted the entry - running `_php_fbird_close_plink`, which `free()`d the `fbird_db_link` struct while request wrappers (`$c1`, `$c2` sharing the le_index_ptr cache entry) still pointed at it. Their later destruction dereferenced freed memory in `_php_fbird_commit_link` (SIGSEGV, verified on PHP 8.5/FB 5.0). Fix: the stale branch now revitalizes the existing struct in place - attach a fresh connection into it after releasing dead transaction/event state (`_php_fbird_commit_link`), keeping entry, struct, and wrapper lifetimes coupled; link counters unchanged (attachment replaced, not added). If the re-attach fails (database still gone), the stale-but-valid entry is kept so surviving wrappers stay safe and the error is reported. Note: the issue's original trigger description (explicit `fbird_close()` freeing `fb_link`) was not reachable on current main - the `le_plink` regular dtor is commit-only (`php_fbird_commit_link_rsrc`); the reachable trigger is any stale-attachment pconnect with live wrappers, verified via `fbird_drop_db()`.
