    ...
    if (event->link != NULL) {
        ...
        if (event->link->handle.ptr != 0 && event->event_id != 0) {
            ...
            if (isc_cancel_events(st, &event->link->handle.db, &event->event_id)) {
                 ...
                 * Do not call _php_fbird_error() from a destructor path for async cleanup. */
            }
        }
 ...
 * _php_fbird_event_block_async below. */
...
static void _php_fbird_event_block_async(unsigned short count, char **events,
...
static void _php_fbird_callback(fbird_event *event, /* {{{ */
...
#endif
{
 ...
 if (!event || event->state == DEAD || !event->link || event->link->handle.ptr == 0) {
	...
	if (event->callback_count >= event->max_callbacks) {
		event->state = DEAD;
		_php_fbird_module_error("Event callback limit exceeded, marking as DEAD");
        return;
    }
...
PHP_FUNCTION(fbird_set_event_handler)
{
 ...
 /* Build buffers for ASYNC registration (no dummy pre-wait) */
    _php_fbird_event_block_async(event->event_count, event->events,
...
PHP_FUNCTION(fbird_free_event_handler)
{
	...
	if (SUCCESS == zend_parse_parameters(ZEND_NUM_ARGS(), "r", &event_arg)) {
		...
		event = (fbird_event *)zend_fetch_resource_ex(event_arg, "Interbase event", le_event);
