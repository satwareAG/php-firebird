			...
			if (!trans && !link) {
				...
				if (trans) {
					...
					if (Z_TYPE_P(arg) == IS_OBJECT) {
						link = _fbird_get_link_from_obj(arg);
					} else {
...
PHP_FUNCTION(fbird_prepare)
{
	...
	if (Z_TYPE(args[i]) == IS_RESOURCE || Z_TYPE(args[i]) == IS_OBJECT) {
		...
		if (trans) {
			...
			if (Z_TYPE(args[i]) == IS_OBJECT) {
				link = _fbird_get_link_from_obj(&args[i]);
			} else {
				link = (fbird_db_link *)zend_fetch_resource_ex(&args[i], NULL, le_link);
				if (!link) link = (fbird_db_link *)zend_fetch_resource_ex(&args[i], NULL, le_plink);
			}
			if (link) {
				link_arg = &args[i];
				i++;
	...
	if (i == 1 && i < argc && (Z_TYPE(args[i]) == IS_RESOURCE || Z_TYPE(args[i]) == IS_OBJECT)) {
		if (trans) {
			fbird_db_link *l = NULL;
			if (Z_TYPE(args[i]) == IS_OBJECT) {
				l = _fbird_get_link_from_obj(&args[i]);
			} else {
				l = (fbird_db_link *)zend_fetch_resource_ex(&args[i], NULL, le_link);
...
PHP_FUNCTION(fbird_execute_auto)
{
    ...
    uintptr_t attachment = fbc_get_attachment_64(link->fbc_connection);
    oo_trans = (fbt_transaction_t*)fbt_start_64(IBG(master_instance), attachment, 0, NULL, IB_STATUS);
    if (!oo_trans) {
        _php_fbird_error();
        RETURN_FALSE;
    }
...
PHP_FUNCTION(fbird_gen_id)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "s|lz!", &generator, &gen_len, &inc, &link) == FAILURE) {
		return;
	}

	if (link == NULL) {
		ib_link = (fbird_db_link *)zend_fetch_resource2(IBG(default_link), LE_LINK, le_link, le_plink);
	} else if (Z_TYPE_P(link) == IS_RESOURCE) {
		ib_link = (fbird_db_link *)zend_fetch_resource2_ex(link, LE_LINK, le_link, le_plink);
	} else {
		ib_link = _fbird_get_link(link);
	}
	if (!ib_link) {
