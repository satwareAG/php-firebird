<?php
// satware-docs/benchmarks/dpb_probe.php
// Probe DPB/attach behaviour for the classic interbase/ibase extension
// against a single Firebird engine.
//
// Usage (from repo root or php*-dev container):
//   php satware-docs/benchmarks/dpb_probe.php
//
// Configuration is taken from config.php, which in turn honours:
//   FB_DSN, FB_HOST, FB_USER, FB_PASSWORD, FB_CHARSET
//
// Typical Docker mappings (see satware-docs/docker-compose.yml):
...
printf("== DPB probe using ibase_connect() ==\n");
printf("Host     : %s\n", $host);
printf("User     : %s\n", $user);
printf("Charset  : %s\n", $charset);

if (function_exists('ibase_get_client_version')) {
    $clientMajor = function_exists('ibase_get_client_major_version') ? ibase_get_client_major_version() : -1;
    $clientMinor = function_exists('ibase_get_client_minor_version') ? ibase_get_client_minor_version() : -1;
    $clientCombined = ibase_get_client_version();
    printf("Client   : major=%d minor=%d combined=%.1f\n", $clientMajor, $clientMinor, $clientCombined);
}

$link = @ibase_connect($host, $user, $password, $charset);

if ($link === false) {
    $errcode = function_exists('ibase_errcode') ? ibase_errcode() : 0;
    $errmsg  = function_exists('ibase_errmsg') ? ibase_errmsg() : 'unknown error';
...
$version = null;
$verRes = @ibase_query($link, "SELECT rdb$get_context('SYSTEM', 'ENGINE_VERSION') FROM RDB$DATABASE");
if ($verRes !== false) {
    $row = ibase_fetch_row($verRes);
    if ($row !== false && isset($row[0])) {
        $version = $row[0];
    }
    ibase_free_result($verRes);
}
