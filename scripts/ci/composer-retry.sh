...
# (packagist dist 429 / codeload 504 during downloads). #604.
#
# Usage: bash scripts/ci/composer-retry.sh install --prefer-dist ...
...
until composer "$@"; do
    ...
    echo "composer $* failed (attempt $attempt/$MAX_ATTEMPTS) - likely transient rate limit/outage (#604), retrying in ${DELAY}s" >&2
    sleep "$DELAY"
    attempt=$((attempt + 1))
    DELAY=$((DELAY * 2))
done
