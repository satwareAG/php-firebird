...
fbird_query($conn, "CREATE PROCEDURE pevent AS BEGIN POST_EVENT 'TEST1'; POST_EVENT 'TEST2'; END");
fbird_commit($conn);

// Register handler for TEST1 then free it - it should NOT fire
$ev = fbird_set_event_handler($conn, 'event_callback', 'TEST1');
fbird_free_event_handler($ev);

// Register handler for TEST2 - it SHOULD fire 5 times then cancel
fbird_set_event_handler($conn, 'event_callback', 'TEST2');
