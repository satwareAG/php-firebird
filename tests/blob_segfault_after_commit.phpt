...
doesn't check for this in _php_ibase_blob_add() before calling isc_put_segment().

Unlike _php_ibase_free_blob() which checks "ib_blob->bl_handle.ptr != 0",
_php_ibase_blob_add() has no such safety check.
--SKIPIF--
