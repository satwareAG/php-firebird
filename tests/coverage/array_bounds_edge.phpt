...
// Create table with various array sizes
// Firebird array syntax: DATATYPE[lower:upper] (no space before bracket)
$create = "
CREATE TABLE ARRAY_BOUNDS_TEST (
    ID INTEGER NOT NULL PRIMARY KEY,
    TINY_ARR INTEGER[1:1],
    MEDIUM_ARR INTEGER[1:10],
    LARGE_ARR INTEGER[1:100]
)";
...
$block = "
EXECUTE BLOCK AS
DECLARE arr INTEGER[1:1];
BEGIN
    arr[1] = 42;
    INSERT INTO ARRAY_BOUNDS_TEST (ID, TINY_ARR) VALUES (1, arr);
END";
...
// Test 2: Medium array fully populated
echo "\n--- Test 2: Medium array (10 elements) ---\n";
$block = "
EXECUTE BLOCK AS
DECLARE arr INTEGER[1:10];
DECLARE i INTEGER;
BEGIN
    i = 1;
    WHILE (i <= 10) DO BEGIN
        ...
        i = i + 1;
    END
    INSERT INTO ARRAY_BOUNDS_TEST (ID, MEDIUM_ARR) VALUES (2, arr);
END";
fbird_query($db, $block);
fbird_commit($db);
...
if (is_array($row['MEDIUM_ARR'])) {
    echo "First: {$row['MEDIUM_ARR'][0]}, Last: {$row['MEDIUM_ARR'][9]}\n";
}
fbird_free_result($r);

// Test 3: Large array partially populated
echo "\n--- Test 3: Large array (100 elements, sparse) ---\n";
$block = "
EXECUTE BLOCK AS
DECLARE arr INTEGER[1:100];
BEGIN
    arr[1] = 1;
    arr[50] = 50;
    arr[100] = 100;
    INSERT INTO ARRAY_BOUNDS_TEST (ID, LARGE_ARR) VALUES (3, arr);
END";
fbird_query($db, $block);
fbird_commit($db);
...
// Test 5: Array with negative values at boundaries
...
BEGIN
    arr[1] = -2147483648;
    arr[10] = 2147483647;
    INSERT INTO ARRAY_BOUNDS_TEST (ID, MEDIUM_ARR) VALUES (5, arr);
END";
fbird_query($db, $block);
fbird_commit($db);
...
$row = fbird_fetch_assoc($r);
if (is_array($row['MEDIUM_ARR'])) {
    echo "ID={$row['ID']}: First={$row['MEDIUM_ARR'][0]}, Last={$row['MEDIUM_ARR'][9]}\n";
}
fbird_free_result($r);
