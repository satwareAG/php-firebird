...
export QUERY_STRING=''
export PATH_TRANSLATED='/ext/tests/coverage/events_error_handling.php'
export SCRIPT_FILENAME='/ext/tests/coverage/events_error_handling.php'
...
case "$1" in
"gdb")
    gdb --args '/usr/local/bin/php'  -n   -d "output_handler=" -d "open_basedir=" -d "disable_functions=" -d "output_buffering=Off" -d "error_reporting=32767" -d "display_errors=1" -d "display_startup_errors=1" -d "log_errors=0" -d "html_errors=0" -d "track_errors=0" -d "report_memleaks=1" -d "report_zend_debug=0" -d "docref_root=" -d "docref_ext=.html" -d "error_prepend_string=" -d "error_append_string=" -d "auto_prepend_file=" -d "auto_append_file=" -d "ignore_repeated_errors=0" -d "precision=14" -d "serialize_precision=-1" -d "memory_limit=128M" -d "opcache.fast_shutdown=0" -d "opcache.file_update_protection=0" -d "opcache.revalidate_freq=0" -d "opcache.jit_hot_loop=1" -d "opcache.jit_hot_func=1" -d "opcache.jit_hot_return=1" -d "opcache.jit_hot_side_exit=1" -d "zend.assertions=1" -d "zend.exception_ignore_args=0" -d "zend.exception_string_param_max_len=15" -d "short_open_tag=0" -d "extension=/ext/modules/firebird.so" -d "extension=pcntl" -d "session.auto_start=0" -d "zlib.output_compression=Off" -f "/ext/tests/coverage/events_error_handling.php"  2>&1
    ;;
"lldb")
