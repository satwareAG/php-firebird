...
$r = fbird_maintain_db($svc, $db_path, FBIRD_RPR_SWEEP_DB, 0);
...
$r = fbird_maintain_db($svc, $db_path, FBIRD_RPR_VALIDATE_DB, 0);
var_dump($r === true || $r === false);
...
$r = fbird_maintain_db($svc, $db_path, FBIRD_PRP_PAGE_BUFFERS, 256);
var_dump($r === true || $r === false);

// 6. Set sweep interval (PRP_SWEEP_INTERVAL)
echo "Test 6: PRP_SWEEP_INTERVAL\n";
$r = fbird_maintain_db($svc, $db_path, FBIRD_PRP_SWEEP_INTERVAL, 20000);
var_dump($r === true || $r === false);

// 7. Set write mode async (PRP_WRITE_MODE)
echo "Test 7: PRP_WRITE_MODE async\n";
$r = fbird_maintain_db($svc, $db_path, FBIRD_PRP_WRITE_MODE, FBIRD_PRP_WM_ASYNC);
var_dump($r === true || $r === false);

// 8. Set write mode sync (restore default)
echo "Test 8: PRP_WRITE_MODE sync\n";
$r = fbird_maintain_db($svc, $db_path, FBIRD_PRP_WRITE_MODE, FBIRD_PRP_WM_SYNC);
var_dump($r === true || $r === false);

// 9. Set access mode readonly
echo "Test 9: PRP_ACCESS_MODE readonly\n";
$r = fbird_maintain_db($svc, $db_path, FBIRD_PRP_ACCESS_MODE, FBIRD_PRP_AM_READONLY);
var_dump($r === true || $r === false);

// 10. Set access mode readwrite (restore default)
echo "Test 10: PRP_ACCESS_MODE readwrite\n";
$r = fbird_maintain_db($svc, $db_path, FBIRD_PRP_ACCESS_MODE, FBIRD_PRP_AM_READWRITE);
var_dump($r === true || $r === false);

// 11. DB info: data pages stats
echo "Test 11: fbird_db_info FBIRD_STS_DATA_PAGES\n";
$r = fbird_db_info($svc, $db_path, FBIRD_STS_DATA_PAGES);
var_dump($r !== false);

// 12. DB info: header pages
echo "Test 12: fbird_db_info FBIRD_STS_HDR_PAGES\n";
$r = fbird_db_info($svc, $db_path, FBIRD_STS_HDR_PAGES);
var_dump($r !== false);
