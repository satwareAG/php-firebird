...
// Create test table with stream BLOB
$result = @fbird_query($db, "DROP TABLE BLOB_SEEK_TEST");
$result = fbird_query($db, "CREATE TABLE BLOB_SEEK_TEST (ID INTEGER PRIMARY KEY, DATA BLOB SUB_TYPE 1)");
fbird_commit($db);
...
// Test 2: Create and write a stream blob with known content
echo "\n=== Test 2: Create blob with known content ===\n";
$trans = fbird_trans($db);
$blob = fbird_blob_create_stream($trans);
if (!$blob) {
    die("Failed to create blob: " . fbird_errmsg());
}

// Write content in chunks: "AAAAAAAAAA" (10 A's) + "BBBBBBBBBB" (10 B's) + "CCCCCCCCCC" (10 C's) = 30 bytes
...
$blob = fbird_blob_open_stream($trans, $blob_id);
if (!$blob) {
...
// Test 6: Error handling - seek on non-stream blob (if applicable)
